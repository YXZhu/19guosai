...
{STEPMOTOR_TIM_PULSE_PIN_Y, STEPMOTOR_TIM_CHANNEL2, STEPMOTOR_TIM_IT_CC2, STEPMOTOR_TIM_FLAG_CC2},//Y轴
  ...
  /* 原点检测引脚*/
const Detect_PIN Origin_Detect[2]={ \
...
__IO uint8_t  ZeroStep[2] = {IDLE,IDLE};	        //搜索原点状态机
__IO uint8_t  LimPosi[2]  = {FALSE,FALSE};      //正转极限标志位  True:到达极限位置  False:未到达极限位置
__IO uint8_t  LimNega[2]  = {FALSE,FALSE};      //反转极限标志位
__IO uint8_t  MotionStatus[2]   = {STOP,STOP};        // 是否在运动？0：停止，1：运动
__IO uint8_t  DOG[2]         		= {FALSE,FALSE};		// 近点信号
__IO uint8_t  HomeCapture[2]    = {FALSE,FALSE};		// 原点捕获标志
__IO int8_t   OriginDir[2] = {CCW,CCW};		// 搜索原点方向 默认逆时针
__IO int32_t  step_position[2]  = {0,0};      // 当前位置  单位:脉冲数
__IO int16_t  location[2]       = {0,0};      // 当前位置  单位:毫米(mm)
...
//    }
    ...
    {
      HAL_NVIC_SetPriority(LimNeg_Detect[i].IRQn, 6, 0);
      HAL_NVIC_EnableIRQ(LimNeg_Detect[i].IRQn);
    }
    if(LimPos_Detect[i].IRQn < EXTI9_5_IRQn)
    {
      HAL_NVIC_SetPriority(LimPos_Detect[i].IRQn, 6, 0);
...
void STEPMOTOR_TIMx_Init()
{
  ...
  sConfigOC.OCNIdleState = TIM_OCNIDLESTATE_RESET;     // 互补通道空闲电平
	...
	/* 配置定时器中断优先级并使能 */
	HAL_NVIC_SetPriority(STEPMOTOR_TIMx_IRQn, 6, 0);
...
{
	__IO  uint8_t static SC_Flag[2] = {0,0};//特殊状况标记位
...
void HAL_TIM_OC_Callback(uint8_t Axis)
{
  ...
  __IO static uint8_t  i[2]={0,0};
	__IO static uint16_t last_accel_delay[2] = {0,0};
	// 总移动步数计数器
	__IO static uint32_t step_count[2] = {0,0};
	// 记录new_step_delay中的余数，提高下一步计算的精度
	__IO static int32_t rest[2] = {0,0};
